(() => {
  ...
  async function importAdvanceRegistrations({eventDate,sourceFile,rows,replacePlanned=true}){const sb=init();const {data,error}=await sb.rpc('admin_import_outreach_preregistrations_v15649',{p_event_date:eventDate,p_source_file:String(sourceFile||''),p_rows:Array.isArray(rows)?rows:[],p_replace_planned:!!replacePlanned,p_user_agent:ua()});if(error)throw error;return data||{};}
  async function getAdvanceRegistrations(eventDate){const sb=init();const d=String(eventDate||'').trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(d))return[];const {data,error}=await sb.from('outreach_preregistrations').select('*').eq('event_date',d).order('source_row',{ascending:true,nullsFirst:false}).order('created_at',{ascending:true});if(error)throw error;return (data||[]).map(r=>({id:r.id,eventDate:r.event_date,sourceKey:r.source_key,sourceFile:r.source_file||'',sourceRow:r.source_row,identityType:r.identity_type||'unknown',idCard:r.id_card||'',passportNumber:r.passport_number||'',prefix:r.prefix||'',fname:r.fname||'',lname:r.lname||'',employeeId:r.employee_id||'',department:r.department||'',phone:r.phone||'',bloodGroup:r.blood_group_reported||'',donorId:r.donor_id_reported||'',lastDonationNo:r.last_donation_no_reported,lastDonationDate:r.last_donation_date_reported||'',birth:r.birth_reported||'',note:r.source_note||'',status:r.status||'planned',registeredDn:r.registered_dn||'',registeredAt:r.registered_at||''}));}
  async function findAdvanceRegistration({eventDate,identityType='thai',idCard='',passportNumber=''}){const rows=await getAdvanceRegistrations(eventDate);const kind=identityType==='passport'?'passport':'thai';if(kind==='passport'){const pass=String(passportNumber||'').toUpperCase().replace(/[^A-Z0-9]/g,'');return rows.find(r=>String(r.passportNumber||'').toUpperCase()===pass)||null;}const id=String(idCard||'').replace(/\D/g,'');return rows.find(r=>String(r.idCard||'').replace(/\D/g,'')===id)||null;}
  async function markRegistrationHistorySource({dn,source,preregistrationId=null}){const sb=init();const {data,error}=await sb.rpc('mark_registration_history_source_v15649',{p_dn:String(dn||''),p_source:String(source||'manual'),p_preregistration_id:preregistrationId||null,p_user_agent:ua()});if(error)throw error;return data||{};}
  async function reconcileAdvanceRegistrations(eventDate){const sb=init();const d=String(eventDate||'').trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(d))return {status:'success',matched:0,matchedNameOnly:0};const {data,error}=await sb.rpc('reconcile_outreach_preregistrations_v15674',{p_event_date:d,p_user_agent:ua()});if(error)throw error;return data||{};}
  async function setAdvanceRegistrationAttendance({id,status}){const sb=init();const {data,error}=await sb.rpc('set_outreach_preregistration_attendance_v15674',{p_preregistration_id:id,p_status:String(status||''),p_user_agent:ua()});if(error)throw error;return data||{};}
