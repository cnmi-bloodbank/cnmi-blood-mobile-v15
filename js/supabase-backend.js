(() => {
  ...
  async function searchDonorsAdvanced(filters={},limit=30){const sb=init();const f=filters||{};const {data,error}=await sb.rpc('search_donor_history_advanced',{p_id_card:String(f.idCard||''),p_donor_id:String(f.donorId||''),p_fname:String(f.fname||''),p_lname:String(f.lname||''),p_phone:String(f.phone||''),p_limit:Math.max(1,Math.min(Number(limit)||30,50))});if(error)throw error;return Array.isArray(data)?data:[];}
  ...
  window.cnmiSupabaseApi={call,client:()=>init(),getDonorHistory,getDonorHistories,importSnapshot,getHistorySummary,searchDonors,searchDonorsAdvanced,getImportedDonationsForDate,resolveDonorIds,createBagRange,getBagBatchStatus,startRealtime};
})();
