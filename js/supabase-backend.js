(() => {
  ...
  async function getBagBatchStatus(outreachDate){
    ...
    return batches.map(b=>{const rows=bags.filter(x=>x.batch_id===b.id),used=rows.filter(x=>x.status==='Used'),available=rows.filter(x=>x.status==='Available'),disabled=rows.filter(x=>x.status==='Disabled');return {...b,bags:rows,used,available,disabled};});
  }
  window.cnmiSupabaseApi={call,client:()=>init(),getDonorHistory,getDonorHistories,importSnapshot,getHistorySummary,searchDonors,searchDonorsAdvanced,getImportedDonationsForDate,resolveDonorIds,setOutreachEvent,getOutreachEvent,importLegacyVisits,reconcileMobileVisits,createBagRange,getBagBatchStatus,startRealtime};
})();
