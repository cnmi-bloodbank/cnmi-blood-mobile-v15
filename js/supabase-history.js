(() => {
  ...
  let advanceDocumentRead=false;
  let advanceDocumentIdentityKey='';
  let advanceIdentityCheck={status:'pending',message:'',method:'',documentRead:false,canManualConfirm:false};
  ...
  const joinAddress=p=>{const sub=cleanArea(p.subdistrict,'subdistrict'),dis=cleanArea(p.district,'district'),pro=cleanArea(p.province,'province'),bkk=pro==='กรุงเทพมหานคร';return [p.addressLine,sub?`${bkk?'แขวง':'ต.'}${sub}`:'',dis?`${bkk?'เขต':'อ.'}${dis}`:'',pro?`${bkk?'':'จ.'}${pro}`:'',p.postalCode].filter(Boolean).join(' ').replace(/\s+/g,' ').trim();};
  ...
  function baseProfile(raw,idCard){const p={idCard,prefix:String(pick(raw,['Prefix','คำนำหน้า'])).trim(),fname:String(pick(raw,['Name','FirstName','ชื่อ'])).trim(),lname:String(pick(raw,['Lastname','LastName','นามสกุล'])).trim(),birth:excelDate(pick(raw,['DOB','BirthDate','วันเกิด'])),gender:genderCode(pick(raw,['Sex','Gender','เพศ'])),addressLine:String(pick(raw,['Address','ที่อยู่'])).trim(),subdistrict:cleanArea(pick(raw,['Tambol','Subdistrict','ตำบล']),'subdistrict'),district:cleanArea(pick(raw,['Amper','District','อำเภอ']),'district'),province:cleanArea(pick(raw,['Province','จังหวัด']),'province'),postalCode:String(pick(raw,['Post','PostalCode','Zip','รหัสไปรษณีย์'])).trim(),phone:phoneText(pick(raw,['Tel','Phone','Telephone','โทรศัพท์'])),email:String(pick(raw,['email','Email','E-mail','อีเมล'])).trim(),occupation:String(pick(raw,['Occupation','อาชีพ'])).trim(),bloodGroupHistory:String(pick(raw,['BloodGroup','Blood Group','หมู่เลือด'])).trim(),donorIds:[],donorIdsAll:[],donationHistory:[],alerts:{notes:[],defers:[],seroNat:[]},_latestRank:0,updatedAt:new Date().toISOString()};p.address=joinAddress(p);return p;}
  ...
  const compactPersonName=v=>String(v||'').trim().toLowerCase().replace(/^(?:นางสาว|น\.ส\.?|นาย|นาง|ด\.ช\.?|ด\.ญ\.?|เด็กชาย|เด็กหญิง)\s*/,'').replace(/[^a-z0-9ก-๙]/g,'');
  function resetAdvanceIdentityCheck(){advanceIdentityVerified=false;verifiedAdvanceIdentityKey='';advanceDocumentRead=false;advanceDocumentIdentityKey='';advanceIdentityCheck={status:'pending',message:'',method:'',documentRead:false,canManualConfirm:false};}
  ...
  function getAdvanceIdentityVerification(){return {...advanceIdentityCheck,verified:isAdvanceIdentityVerified()};}
  function invalidateAdvanceIdentityVerification(){
    if(!activeAdvanceRegistration)return;
    resetAdvanceIdentityCheck();
    window.updateAdvanceRegisterBanner?.(activeAdvanceRegistration);
  }
  function compareAdvanceDemographics(actualProfile){
    const a=activeAdvanceRegistration;if(!a)return {ok:true,message:''};
    if((a.identityType||'thai')==='passport')return {ok:true,message:'Passport จริงยืนยันแล้ว'};
    const ef=compactPersonName(a.fname),el=compactPersonName(a.lname),eb=valueToYmd(a.birth);
    if(!ef||!el||!eb)return {ok:false,pending:true,message:'รายชื่อแจ้งล่วงหน้าขาดชื่อ/นามสกุล/วันเกิด จึงยังใช้ประวัติกาชาดไม่ได้'};
    const af=compactPersonName(actualProfile?.fname||actualProfile?.firstName),al=compactPersonName(actualProfile?.lname||actualProfile?.lastName),ab=valueToYmd(actualProfile?.birthDate||actualProfile?.birth);
    if(!af||!al||!ab)return {ok:false,pending:true,message:'อ่านบัตรจริงแล้ว แต่ยังอ่านชื่อ/นามสกุล/วันเกิดอัตโนมัติไม่ครบ'};
    const dif=[];if(ef!==af)dif.push('ชื่อ');if(el!==al)dif.push('นามสกุล');if(eb!==ab)dif.push('วันเกิด');
    if(dif.length)return {ok:false,mismatch:true,message:`${dif.join(' / ')} จากบัตรจริงไม่ตรงกับรายชื่อที่เลือก`};
    return {ok:true,message:'ชื่อ + นามสกุล + วันเกิดตรงกับรายชื่อที่เลือก'};
  }
  function markAdvanceIdentityVerified({method='manual',actualProfile=null}={}){
    ...
    if(method==='staff-confirm'){
      if(!advanceDocumentRead||!advanceDocumentIdentityKey||advanceDocumentIdentityKey!==key){
        advanceIdentityCheck={status:'pending',message:'กรุณาอ่านบัตรจริงก่อนยืนยันด้วยตา',method,documentRead:false,canManualConfirm:false};
        window.updateAdvanceRegisterBanner?.(activeAdvanceRegistration);return false;
      }
      advanceIdentityVerified=true;verifiedAdvanceIdentityKey=key;
      advanceIdentityCheck={status:'verified',message:'เจ้าหน้าที่ตรวจชื่อ + นามสกุล + วันเกิดกับบัตรจริงแล้ว',method,documentRead:true,canManualConfirm:false};
    }else{
      advanceDocumentRead=true;advanceDocumentIdentityKey=key;
      const cmp=compareAdvanceDemographics(actualProfile);
      if(cmp.ok){
        advanceIdentityVerified=true;verifiedAdvanceIdentityKey=key;
        advanceIdentityCheck={status:'verified',message:cmp.message||'',method,documentRead:true,canManualConfirm:false};
      }else{
        advanceIdentityVerified=false;verifiedAdvanceIdentityKey='';
        advanceIdentityCheck={status:cmp.mismatch?'mismatch':'pending',message:cmp.message||'',method,documentRead:true,canManualConfirm:true};
      }
    }
    activeAdvanceRegistration={...activeAdvanceRegistration,identityVerified:isAdvanceIdentityVerified(),verifiedIdentityKey:isAdvanceIdentityVerified()?key:'',verifyMethod:method,identityCheckStatus:advanceIdentityCheck.status};
    window.updateAdvanceRegisterBanner?.(activeAdvanceRegistration);
    return isAdvanceIdentityVerified();
  }
  ...
  function advanceSourceBox(p){
    ...
    const parts=[];
    if(String(a.donorId||'').trim())parts.push(`Donor ID <strong>${escapeHtml(String(a.donorId))}</strong>`);
    if(Number.isInteger(Number(a.lastDonationNo)))parts.push(`เดิม <strong>${Number(a.lastDonationNo)} ครั้ง</strong>`);
    if(!parts.length)return null;
    const box=document.createElement('div');box.className='preload-alert preload-note py-2';
    box.innerHTML=`📋 ${parts.join(' · ')}`;return box;
  }
  ...
  function renderProfile(p){
    ...
    const advanceOnly=p.historySource==='advance-list'&&!hist.length&&!mobile.length;if(advanceOnly)return;
    ...
    const title=document.createElement('div');title.className='preload-history-title';title.textContent=(p.historySource==='advance-list'&&!hist.length&&!mobile.length)?'พบรายชื่อผู้บริจาคแจ้งล่วงหน้า':'พบประวัติผู้บริจาคเดิม';summary.appendChild(title);
    const stack=document.createElement('div');stack.className='preload-photo-row';
    const summaryStack=document.createElement('div');summaryStack.className='preload-summary-stack';
  ...
  async function findDonor(id){const clean=String(id||'').replace(/\D/g,'');if(!/^\d{13}$/.test(clean))return null;const key='thai:'+clean;if(mem.has(key))return mem.get(key);const p=await window.cnmiSupabaseApi.getDonorHistory(clean);if(p){p.identityType='thai';p.identityKey=clean;mem.set(key,p);}return p;}
  async function findDonorPassport(passport){const pass=String(passport||'').toUpperCase().replace(/[^A-Z0-9]/g,'');if(!pass)return null;const key='passport:'+pass;if(mem.has(key))return mem.get(key);const p=await window.cnmiSupabaseApi.getDonorByPassport(pass);if(p)mem.set(key,p);return p;}
  async function findMany(ids){const keys=[...new Set((ids||[]).map(x=>String(x||'').trim()).filter(x=>/^\d{13}$/.test(x)||/^LEGACY:[0-9a-f]{32}$/i.test(x)))];const result={};const thai=keys.filter(x=>/^\d{13}$/.test(x));const missingThai=thai.filter(x=>!mem.has('thai:'+x));if(missingThai.length){const got=await window.cnmiSupabaseApi.getDonorHistories(missingThai);Object.entries(got||{}).forEach(([k,v])=>mem.set('thai:'+k,v));}thai.forEach(k=>{if(mem.has('thai:'+k))result[k]=mem.get('thai:'+k);});const other=keys.filter(x=>/^LEGACY:/i.test(x));if(other.length){try{const r=await window.cnmiSupabaseApi.call('getDonorsByIds',{data:JSON.stringify({ids:other})});for(const d of (r?.donors||[]))result[d.identityKey||d.idCard]=d;}catch{}}return result;}
  ...
  function registrationSuggestion(p){
    ...
    const advanceDonor=String(p?.advanceRegistration?.donorId||'').trim();
    // CNMI เป็นสาขากาชาด: ถ้ามี Donor ID ที่เช็กไว้ล่วงหน้าให้เตรียมใช้ได้
    // แต่ถ้าอ่านบัตรจริงแล้วพบ AI-LIS/Blood Mobile ระบบของเราจะใช้ค่าที่พบจริงก่อนเสมอ
    ...
    const mobileNums=mobile.map(x=>Number(String(x?.donationNo||'').replace(/\.0+$/,''))).filter(n=>Number.isInteger(n)&&n>0);
    if(mobileNums.length)return {donorId:String(donorId||''),lastDonationNo:String(Math.max(...mobileNums)),historySource:'blood-mobile'};
    const advanceNo=Number(p?.advanceRegistration?.lastDonationNo);
    if(Number.isInteger(advanceNo)&&advanceNo>=0)return {donorId:String(donorId||''),lastDonationNo:String(advanceNo),historySource:'advance-list'};
    return {donorId:String(donorId||''),lastDonationNo:'0',historySource:'manual'};
  }
  function applyAdvanceRegistration(a,{force=false}={}){
    if(!a)return null;
    setCentralProfileLock(null,false);
    activeAdvanceRegistration=a;resetAdvanceIdentityCheck();
    ...
    if(a.identityType==='passport'){window.setRegisterIdentityType?.('passport',{clear:false});const id=$('passport_number');if(id)id.value='';}else{window.setRegisterIdentityType?.('thai',{clear:false});const id=$('id_card');if(id)id.value='';}
    ...
    if(donorEl){donorEl.value=sug.donorId||'';donorEl.dataset.userEdited='0';}
    ...
    setText('preload-register-status','เลือกจากรายชื่อแล้ว · อ่านบัตรจริงเพื่อยืนยัน','register-status-line text-warning fw-bold');
  ...
  async function lookupFromRegister({fill=true}={}){
    ...
    if(kind==='passport'){
      ...
      if(identityValue.length<5){clearPanel();if(!selectedBefore){activeAdvanceRegistration=null;resetAdvanceIdentityCheck();currentRegistrationHistorySource='manual';window.updateAdvanceRegisterBanner?.(null);}window.resetDonorPhotoForIdentityChange?.();setText('preload-register-status',selectedBefore?'รายชื่อแจ้งล่วงหน้ายังรอยืนยัน Passport จริง':'','register-status-line text-muted');return null;}
      p=await findDonorPassport(identityValue);
      advance=selectedBefore||null;
    }else{
      identityValue=String($('id_card')?.value||'').replace(/\D/g,'');
      if(!/^\d{13}$/.test(identityValue)){clearPanel();if(!selectedBefore){activeAdvanceRegistration=null;resetAdvanceIdentityCheck();currentRegistrationHistorySource='manual';window.updateAdvanceRegisterBanner?.(null);}window.resetDonorPhotoForIdentityChange?.();setText('preload-register-status',selectedBefore?'รายชื่อแจ้งล่วงหน้ายังรออ่านบัตรจริง':'','register-status-line text-muted');return null;}
      // v15.6.70: ค้น AI-LIS/Blood Mobile ด้วยเลขจากบัตรจริงเสมอ และไม่ auto-link รายชื่อด้วยเลขประชาชนจากกาชาด
      p=await findDonor(identityValue);
      advance=selectedBefore||null;
    }
    const hadCentral=!!p;
    activeAdvanceRegistration=advance||null;
    if(p){p={...p,advanceRegistration:advance||null};}
    else if(advance&&selectedIsVerified){p=advanceToProfile(advance);}
    if(!p){
      clearPanel();currentRegistrationHistorySource='manual';if(!window.cnmiDonorPhotoState?.dirty)window.resetDonorPhotoForIdentityChange?.();
      const donorEl=$('donor_id'),noEl=$('donation_no');
      if(advance){const ap=advanceToProfile(advance),sug=registrationSuggestion(ap);if(donorEl&&donorEl.dataset.userEdited!=='1')donorEl.value=sug.donorId||'';if(noEl&&noEl.dataset.userEdited!=='1'){noEl.value=sug.lastDonationNo||'0';window.updateDonationNoHint?.();}}
      else{if(donorEl&&donorEl.dataset.userEdited!=='1')donorEl.value='';if(noEl&&noEl.dataset.userEdited!=='1'){noEl.value='0';window.updateDonationNoHint?.();}}
      const msg=advance&&!selectedIsVerified?'รออ่านบัตรจริงเพื่อยืนยัน':(kind==='passport'?'ไม่พบประวัติ Passport':'ไม่พบประวัติเดิม');
      setText('preload-register-status',msg,'register-status-line text-secondary');window.updateAdvanceRegisterBanner?.(advance);return null;
    }
    p.donorReactions=[];
    if(hadCentral||!activeAdvanceRegistration||isAdvanceIdentityVerified()){
      try{const rx=await window.cnmiSupabaseApi.call('getDonorReactions',{data:JSON.stringify({identityKey:p.identityKey||identityValue,limit:20})});if(rx?.status==='success'&&Array.isArray(rx.reactions))p.donorReactions=rx.reactions;}catch(err){console.warn('load donor reactions failed',err);}
    }
    ...
    renderProfile(p);setCentralProfileLock(p,currentRegistrationHistorySource==='ai-lis');
    const alerts=(p.alerts?.notes?.length||0)+(p.alerts?.defers?.length||0)+(p.alerts?.seroNat?.length||0)+(p.donorReactions?.length||0);
    const sourceLabel=currentRegistrationHistorySource==='ai-lis'?'AI-LIS':currentRegistrationHistorySource==='blood-mobile'?'Blood Mobile':currentRegistrationHistorySource==='advance-list'?'รายชื่อแจ้งล่วงหน้า':'ข้อมูลที่กรอก';
    ...
    if(activeAdvanceRegistration&&!isAdvanceIdentityVerified())message='รออ่านบัตรจริงเพื่อยืนยัน';
  ...
  window.cnmiPreload={findDonor,findDonorPassport,findMany,upsertBasic,getSummary:refreshSummary,hasData:async()=>!!(await window.cnmiSupabaseApi.getHistorySummary()),refreshSummary,lookupFromRegister,renderProfile,clearPanel,applyAdvanceRegistration,getActiveAdvanceRegistration:()=>activeAdvanceRegistration,getRegistrationHistorySource:()=>currentRegistrationHistorySource,isAdvanceIdentityVerified,getAdvanceIdentityVerification,markAdvanceIdentityVerified,invalidateAdvanceIdentityVerification,confirmAdvanceIdentityForLookup:async()=>{if(!markAdvanceIdentityVerified({method:'staff-confirm'}))return alert('กรุณาอ่านบัตรจริงก่อน แล้วตรวจชื่อ + นามสกุล + วันเกิดบนบัตรให้ตรงกับรายชื่อที่เลือก');await lookupFromRegister({fill:true});},continueWithoutAdvanceRegistration:async()=>{const a=activeAdvanceRegistration;const same=(x,y)=>String(x||'').trim()===String(y||'').trim();if(a){[['prefix',a.prefix],['fname',a.fname],['lname',a.lname],['birth_date',a.birth],['phone',a.phone]].forEach(([id,v])=>{const el=$(id);if(el&&same(el.value,v))el.value='';});}activeAdvanceRegistration=null;resetAdvanceIdentityCheck();currentRegistrationHistorySource='manual';setCentralProfileLock(null,false);window.updateAdvanceRegisterBanner?.(null);await lookupFromRegister({fill:true});},clearAdvanceRegistration:()=>{activeAdvanceRegistration=null;resetAdvanceIdentityCheck();currentRegistrationHistorySource='manual';setCentralProfileLock(null,false);window.updateAdvanceRegisterBanner?.(null);}};
...
${built.summary.sourceThroughDateThai?`ข้อมูลรายชื่อผู้บริจาคถึง ${built.summary.sourceThroughDateThai}\n`:''}รอบนี้จับคู่คำเตือนไม่ได้ ${Number(built.summary.unmatchedAlertDonorIds||0).toLocaleString()} Donor_ID`);}catch(err){console.error(err);setText('preload-import-status',`นำเข้าไม่สำเร็จ: ${err.message||err}`,'small fw-bold text-danger mt-2');alert(`นำเข้าไม่สำเร็จ\n${err.message||err}`);}finally{if(btn){btn.disabled=false;btn.textContent='อัปเดตฐานกลาง 4 Reports';}}};
  ...
  window.exportOnlineLisCsv=async()=>{try{
    const date=$('lis-export-date')?.value||$('table-date')?.value||localYmd(),rows=await getVisitDataFast(date,true);
    if(!Array.isArray(rows)||!rows.length)return alert('ไม่มีข้อมูลของวันที่เลือก');
    const profiles=await findMany(rows.map(r=>r.identityKey||r.dbIdCard||r.idCard));
    const prof=r=>profiles[r.identityKey||r.dbIdCard||r.idCard]||{};
    const cols=[
      ['DN',r=>r.dn],['Identity_Type',r=>r.identityType==='passport'?'PASSPORT':'THAI'],['ID_Card',r=>r.identityType==='passport'?'':r.idCard],['Passport_Number',r=>r.passportNumber||''],['Nationality',r=>r.nationality||''],
      ['Prefix',r=>(prof(r).prefix||r.prefix||'')],['FirstName',r=>(prof(r).fname||r.fname||'')],['LastName',r=>(prof(r).lname||r.lname||'')],['Birthdate',r=>(prof(r).birth||r.birth||'')],['Gender',r=>genderCode(prof(r).gender||r.gender||'')],
      ['Address',r=>(prof(r).addressLine||r.addressLine||prof(r).address||r.address||'')],['Subdistrict',r=>cleanArea(prof(r).subdistrict||r.subdistrict||'','subdistrict')],['District',r=>cleanArea(prof(r).district||r.district||'','district')],['Province',r=>cleanArea(prof(r).province||r.province||'','province')],['Postal_Code',r=>(prof(r).postalCode||r.postalCode||'')],
      ['Occupation',r=>(r.occupation||prof(r).occupation||'')],['Phone',r=>(prof(r).phone||r.phone||'')],['Email',r=>(prof(r).email||'')],['Weight',r=>r.weight],['BP',r=>r.bp],['Pulse',r=>r.pulse],['Temperature',r=>r.temp],['Hb',r=>r.hb],['Blood_Group',r=>r.group],['Bag_Number',r=>r.bag==='-'?'':r.bag],['Bag_Lot',r=>r.bagLot||''],['Donor_Type',r=>r.type],['Screening_Status',r=>r.status],['Reason',r=>r.reason],['Save_Time',r=>r.saveTime],['C1',r=>r.c1],['C2',r=>r.c2],['E1',r=>r.e1],['E2',r=>r.e2],['Donor_ID',r=>r.donorId||''],['Donation_No',r=>r.donationNo||''],['Visit_Date',r=>date],['Location',r=>r.location||''],['Last_Donation_No',r=>{const raw=String(r.donationNo??'').trim();if(!raw)return'';const n=Number(raw);return Number.isFinite(n)?Math.max(0,n-1):'';}]
    ];
