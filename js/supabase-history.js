...
${built.summary.sourceThroughDateThai?`ข้อมูลรายชื่อผู้บริจาคถึง ${built.summary.sourceThroughDateThai}\n`:''}รอบนี้จับคู่คำเตือนไม่ได้ ${Number(built.summary.unmatchedAlertDonorIds||0).toLocaleString()} Donor_ID`);}catch(err){console.error(err);setText('preload-import-status',`นำเข้าไม่สำเร็จ: ${err.message||err}`,'small fw-bold text-danger mt-2');alert(`นำเข้าไม่สำเร็จ\n${err.message||err}`);}finally{if(btn){btn.disabled=false;btn.textContent='อัปเดตฐานกลาง 4 Reports';}}};
  ...
  window.exportOnlineLisCsv=async()=>{try{const date=$('lis-export-date')?.value||$('table-date')?.value||localYmd(),rows=await getVisitDataFast(date,true);if(!Array.isArray(rows)||!rows.length)return alert('ไม่มีข้อมูลของวันที่เลือก');const profiles=await findMany(rows.map(r=>r.idCard));const cols=[['DN',r=>r.dn],['ID_Card',r=>r.idCard],['Prefix',r=>(profiles[r.idCard]?.prefix||r.prefix||'')],['FirstName',r=>(profiles[r.idCard]?.fname||r.fname||'')],['LastName',r=>(profiles[r.idCard]?.lname||r.lname||'')],['Birthdate',r=>(profiles[r.idCard]?.birth||r.birth||'')],['Gender',r=>genderCode(profiles[r.idCard]?.gender||r.gender||'')],['Address',r=>(profiles[r.idCard]?.addressLine||r.addressLine||profiles[r.idCard]?.address||r.address||'')],['Subdistrict',r=>cleanArea(profiles[r.idCard]?.subdistrict||r.subdistrict||'','subdistrict')],['District',r=>cleanArea(profiles[r.idCard]?.district||r.district||'','district')],['Province',r=>cleanArea(profiles[r.idCard]?.province||r.province||'','province')],['Postal_Code',r=>(profiles[r.idCard]?.postalCode||r.postalCode||'')],['Occupation',r=>(r.occupation||profiles[r.idCard]?.occupation||'')],['Phone',r=>(profiles[r.idCard]?.phone||r.phone||'')],['Email',r=>(profiles[r.idCard]?.email||'')],['Weight',r=>r.weight],['BP',r=>r.bp],['Pulse',r=>r.pulse],['Temperature',r=>r.temp],['Hb',r=>r.hb],['Blood_Group',r=>r.group],['Bag_Number',r=>r.bag==='-'?'':r.bag],['Bag_Lot',r=>r.bagLot||''],['Donor_Type',r=>r.type],['Screening_Status',r=>r.status],['Reason',r=>r.reason],['Save_Time',r=>r.saveTime],['C1',r=>r.c1],['C2',r=>r.c2],['E1',r=>r.e1],['E2',r=>r.e2],['Donor_ID',r=>r.donorId||''],['Donation_No',r=>r.donationNo||''],['Visit_Date',r=>date],['Location',r=>r.location||''],['Last_Donation_No',r=>{const raw=String(r.donationNo??'').trim();if(!raw)return'';const n=Number(raw);return Number.isFinite(n)?Math.max(0,n-1):'';}]];downloadCsv(cols.map(c=>csvEscape(c[0])).join(',')+'\r\n'+rows.map(r=>cols.map(c=>csvEscape(c[1](r)??'')).join(',')).join('\r\n'),`CNMI-LIS-${date}.csv`);}catch(err){console.error(err);alert(`ส่งออก LIS CSV ไม่สำเร็จ\n${err.message||err}`);}};
  let debounce=null;document.addEventListener('DOMContentLoaded',()=>{$('id_card')?.addEventListener('input',()=>{clearTimeout(debounce);clearPanel();window.resetDonorPhotoForIdentityChange?.();const donorEl=$('donor_id'),noEl=$('donation_no');if(donorEl&&donorEl.dataset.userEdited!=='1')donorEl.value='';if(noEl&&noEl.dataset.userEdited!=='1'){noEl.value='0';window.updateDonationNoHint?.();}try{const occ=$('occupation_type');if(occ&&occ.dataset.userEdited!=='1')window.setOccupationUiFromStored?.('',true);}catch{}debounce=setTimeout(()=>lookupFromRegister({fill:true}).catch(console.warn),120);});$('id_card')?.addEventListener('blur',()=>lookupFromRegister({fill:true}).catch(console.warn));setTimeout(()=>refreshSummary().catch(()=>{}),500);});
})();
